...
Emacs by itself does not have many packages.
We add to the list two more repositories, =melpa= and =org=.
...
To be able to manage configurations with =use-package=, it is necessary to
first install it, if we haven't already.
...
#+begin_src emacs-lisp :tangle yes
  (setq-default
   ...
   c-basic-offset 4                                     ; Set indent width in C/C++ at 4 spaces
   c-default-style "bsd")                               ; Set C/C++ style to Allman (BSD)
...
as LSP client and [[https://github.com/Microsoft/python-language-server][mspyls]] as LSP server.

*NOTE:* before configuring =lsp-python-ms=, don't forget to configure [[#lsp][lsp-mode]].
